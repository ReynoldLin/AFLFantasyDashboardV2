...
function PlayerSearchBox({ label, selectedPlayer, onSelect, disabled, disabledHint, players, loading }) {
  ...
  return (
    <div ref={wrapperRef} style={{ position: 'relative', width: 280 }}>
      ...
      {selectedPlayer ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--surface)', border: '2px solid var(--accent)', borderRadius: 8, padding: '8px 12px' }}>
          <img
          ...
          <button
            onClick={handleClear}
            style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--muted)', fontSize: 16, lineHeight: 1, padding: '0 2px' }}
          >
            ✕
          </button>
        </div>
        ...
        <div style={{ position: 'relative' }}>
          <input
            ...
            style={{
              ...
              background: disabled ? 'var(--surface2)' : undefined,
            }}
            type="text"
          ...
          {search && !disabled && (
            <button
              ...
              style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--muted)', fontSize: 14, padding: 0, lineHeight: 1 }}
            >
              ✕
            </button>
          )}
...
export function WithWithout() {
  ...
  // Players in the same squad as playerWith, excluding playerWith themselves
  ...
  return (
    <div style={{ padding: 32 }}>
      <main>
        <div className="empty">
        Search for a player below to compare team performance with and without them
        </div>
        <div className="filters" style={{ alignItems: 'flex-start', gap: 24 }}>
          ...
          <div style={{ alignSelf: 'center', color: 'var(--muted)', fontSize: 18, fontWeight: 300, paddingTop: 22 }}>
            /
          </div>
        ...
        {playerWith && playerWithout && (
          <div style={{ marginTop: 24, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 32, textAlign: 'center', color: 'var(--muted)', fontSize: 14 }}>
            Stats for{' '}
            <strong style={{ color: 'var(--text)' }}>{playerWith.firstName} {playerWith.lastName}</strong>
            {' '}/{' '}
            <strong style={{ color: 'var(--text)' }}>{playerWithout.firstName} {playerWithout.lastName}</strong>
            {' '}coming soon.
          </div>
        )}
      </main>
